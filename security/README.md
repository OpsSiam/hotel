...
- `03_patch_management.sh` enable `dnf-automatic` for security updates
  - [OK] Safe baseline; low risk of SSH lockout.
- `04_user_policy.sh` password aging and sudo policy
  - [!] High risk: can block access if sudo/wheel user not created and tested first.
- `05_ssh_hardening.sh` SSH configuration hardening
...
Create a wheel/sudo user and confirm SSH key login works before running `05_ssh_hardening.sh` or `04_user_policy.sh`.
...
sudo ./04_user_policy.sh
sudo ./05_ssh_hardening.sh
```
